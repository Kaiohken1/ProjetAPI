...
class ReservationController {
    ...
    function createReservation($req, $res) {
        ...
        if (empty($req->body->dateDebut) || empty($req->body->dateFin) || empty($userId) || empty($req->body->prix)) {
            $res->status = 400;
            $res->content = json_encode(['error' => 'Données de réservation manquantes.']);
            return;
        }
        ...
        try {
            $newReservation = $this->service->createReservation($reservation);
